...
|------|----------|---------------------|-------|
| Member | `***REMOVED***` | `MEMBER_PASSWORD` | Access to community hub, bulletin, calendar, chat, etc. |
| Admin | `***REMOVED***` | `ADMIN_PASSWORD` | Access to administrator dashboard, moderation tools |
...
3. User enters password: `***REMOVED***`
...
2. User enters admin password: `***REMOVED***`
...
# Example: MEMBER_PASSWORD=***REMOVED***
MEMBER_PASSWORD=
...
# Example: ADMIN_PASSWORD=***REMOVED***
ADMIN_PASSWORD=
...
**Symptoms**: User enters `***REMOVED***` but gets "Incorrect password."
...
4. **Check for typos**:
   - Ensure password is exactly `***REMOVED***` (case-sensitive)
   - No extra spaces (client-side `.trim()` handles this)
   - Special characters are correct (`!` at the end)

**Solution**: Set `MEMBER_PASSWORD=***REMOVED***` in `.env.local` and restart server.
...
- Member password: `***REMOVED***` (12 characters, mixed case, special char)
- Admin password: `***REMOVED***` (7 characters, mixed case, special char)
...
- [ ] Set `MEMBER_PASSWORD` (current: `***REMOVED***`)
- [ ] Set `ADMIN_PASSWORD` (current: `***REMOVED***`)
- [ ] Verify `.env.local` is NOT committed to Git
- [ ] Test member login with `***REMOVED***`
- [ ] Test admin login with `***REMOVED***`
